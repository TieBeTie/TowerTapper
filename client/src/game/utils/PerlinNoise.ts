...
/**
 * Простая реализация 2-D шума Перлина (public-domain)
 * Используется для лёгкой хаотичности траекторий частиц.
 */
export default class PerlinNoise {
    ...
    private static lerp(a: number, b: number, t: number) { return a + t * (b - a); }
    private static grad(hash: number, x: number, y: number) { const h = hash & 3; const u = h < 2 ? x : y; const v = h < 2 ? y : x; return ((h & 1) === 0 ? u : -u) + ((h & 2) === 0 ? v : -v); }
